...
regressor = GradientBoostingRegressor(n_estimators = 300, max_depth=7, verbose = 2)
regressor.fit(data.values, labels)
...
ass_list = np.unique(X['ASS_ASSIGNMENT'].values)
for ass in ass_list :
    X["AA_"+ass] = (X['ASS_ASSIGNMENT'] == ass)*1


submission.prediction=regressor.predict(X.drop(['ASS_ASSIGNMENT','prediction'], axis=1))
